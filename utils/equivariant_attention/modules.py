...
from utils.equivariant_attention.from_se3cnn import utils_steerable
from utils.equivariant_attention.fibers import Fiber, fiber2head
from utils.utils.utils_logging import log_gradient_norm
        ...
        for d_in in range(max_degree+1):
            for d_out in range(max_degree+1):
                ...
                for J in range(abs(d_in-d_out), d_in+d_out+1):
                    ...
                    # Q_J = Q_J.half().T.to(device)

                    # Create kernel from spherical harmonics
                    # import pdb
                    # pdb.set_trace()
                    Y[J] = Y[J].to(torch.float)
                    Q_J = Q_J.to(torch.float)
    ...
    def forward(self, features, **kwargs):
        ...
        for k, v in features.items():
            if str(k) in self.transform.keys():
                output[k] = torch.matmul(self.transform[str(k)].float(), v.float())
                # output[k] = torch.matmul(self.transform[str(k)].half(), v.half())
        return output
...
class GAttentiveSelfInt(nn.Module):
    ...
    def forward(self, features, **kwargs):
        ...
        for k, v in features.items():
            ...
            # Compute the norms and normalized features
            scalars = torch.einsum('...ac,...bc->...ab', [v, v])
            scalars = scalars.view(*first_dims, m_in*m_in)
            sign = scalars.sign()
            scalars = scalars.abs_().clamp_min(self.eps)
            scalars = scalars * sign

            # perform attention
            att_weights = self.transform[str(k)](scalars) 
            att_weights = att_weights.view(*first_dims, m_out, m_in)
