...
import math
import numpy as np
from utils.equivariant_attention.from_se3cnn.SO3 import irr_repr, torch_default_dtype
from utils.equivariant_attention.from_se3cnn.cache_file import cached_dirpklgz
from utils.equivariant_attention.from_se3cnn.representations import SphericalHarmonics
...
def _basis_transformation_Q_J(J, order_in, order_out, version=3):  # pylint: disable=W0613
    ...
    with torch_default_dtype(torch.float32):
        ...
        print("2*od_out + 1",2 * order_out + 1)
        print("2*od_in + 1", (2 * order_in + 1))
