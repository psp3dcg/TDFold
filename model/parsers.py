'''
A3m, hhr, pdb, template parsing functions
'''

import numpy as np
import string
import re
import model.aa_info_util as aa_info_util
import torch
from model.ffindex import *
to1letter = {
...
def parse_pdb(filename):
    ...
    Input:
        - filename(str):pdb path
    Output:
        - xyz(tensor):N,Ca,C atom position
...
def parse_pdb_lines(lines):
    '''parse lines in pdb file

    Input:
        - lines(list):lines of pdb file
    Output:
        - xyz(np.array):atom positions
        - mask(np.array):atom mask
        - np.array(idx_s):residue index
    '''
    # indices of residues observed in the structure
    ...
    for l in lines:
        ...
        for i_atm, tgtatm in enumerate(aa_info_util.aa2long[aa_info_util.aa2num[aa]]):
            if tgtatm == atom:
...
def parse_templates(ffdb, hhr_fn, atab_fn, n_templ=10):
    ...
    Input:
        - ffdb(file):ff database file object
        - hhr_fn(file):hhr file object
        - atab_fn(file):atab file object
        - n_templ(int):number of templates
    Output:
        - torch.from_numpy(xyz)(tensor):template atom position
        - torch.from_numpy(qmap)(tensor):qi hit map
        - torch.from_numpy(f0d)(tensor):0-dimension feature
        - torch.from_numpy(f1d)(tensor):1-dimension feature
        - ids(tensor):id set
    '''
...
def read_templates(qlen, ffdb, hhr_fn, atab_fn, n_templ=10):
    ...
    Input:
        - qlen(int):qmap length
        - ffdb(file):ff database file object
        - hhr_fn(file):hhr file object
        - atab_fn(file):atab file object
        - n_templ(int):number of templates
    Output:
        - xyz(tensor):template atom positions
        - f1d(tensor):1-dimension feature
        - torch.stack(f0d, dim=0)(tensor):0-dimension feature
    '''
