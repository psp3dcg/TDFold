...
import math
import random
from model.Transformer import EncoderLayer, AxialEncoderLayer, Encoder, LayerNorm
...
class PositionalEncoding(nn.Module):
    ...
    Input:
        ...
        - max_len(int): residue index from ground truth pdb
    '''
...
class Templ_emb(nn.Module):
    ...
    def forward(self, t1d, t2d, idx, test_flag=False):
        # Input
        ...
        if not test_flag:
            ...
            if rand_flag > 0:
                templ_idx = torch.randperm(t1d.shape[1])
                t1d = t1d[:, templ_idx, :, :].view(t1d.shape)
                t2d = t2d[:, templ_idx, :, :, :].view(t2d.shape)
        B, T, L, _ = t1d.shape
...
class Pair_emb_w_templ(nn.Module):
    ...
    def forward(self, seq, idx, templ):
        ...
        # seqsep = torch.log(seqsep.half()).view(B,L,L,1)
        #
