...
def lddt(predicted_points,
    ...
    distance difference tests. Bioinformatics 29, 2722�C2728 (2013).
    ...
    dists_to_score = (
            jnp.matmul((dmat_true < cutoff).float(), (1. - jnp.eye(dmat_true.shape[1])).to(args.device))
            )
...
if __name__ == '__main__':
    ...
    predicted_points = jnp.randn(3, 5, 3, requires_grad=True).to('cuda')
    true_points = jnp.randn(3, 5, 3).to('cuda')
