...
def xyz_to_t2d(xyz_t, t0d, params=PARAMS):
    ...
    if len(list(t0d.size())) < 3:
        t0d = t0d.unsqueeze(0)
    # print(t0d.unsqueeze(2).unsqueeze(3).size())
    t0d = t0d.unsqueeze(2).unsqueeze(3).expand(-1, -1, L, L, -1)
    idx = torch.arange(L, device=xyz_t.device).long().expand((B, L))
    sep = idx[:,None,:] - idx[:,:,None]
    sep = sep.abs() + torch.eye(L, device=xyz_t.device).unsqueeze(0)*999.9
    topk_matrix = torch.zeros((B, L, L), device=xyz_t.device)
    cond = torch.logical_or(topk_matrix > 0.0, sep < (L//4))
    b,i,j = torch.where(cond)
    topk_matrix[b,i,j]=1
    topk_matrix = topk_matrix.unsqueeze(1).unsqueeze(-1)
    dist = topk_matrix*dist
    orien = topk_matrix*orien
    #
    t2d = torch.cat((dist, orien, t0d), dim=-1)
    t2d[torch.isnan(t2d)] = 0.0
    return t2d

def xyz_to_t2d_new(xyz_t, params=PARAMS):
    """convert template cartesian coordinates into 2d distance 
    and orientation maps
    ...
    orien = topk_matrix*orien
    #
    t2d = torch.cat((dist, orien), dim=-1)
    t2d[torch.isnan(t2d)] = 0.0
    return t2d


def xyz_to_t2d_dist(xyz_t, params=PARAMS):
    """convert template cartesian coordinates into 2d distance 
    and orientation maps
    ...
    # dist = c6d[...,:1]*mask / params['DMAX'] # from 0 to 1 # (B, T, L, L, 1)
    # dist = torch.clamp(dist, 0.0, 1.0)
    dist = dist_to_onehot(c6d[...,:1]*mask, params).squeeze(-2)
...
def c6d_to_t2d_dist(c6d, params=PARAMS):
    def dist_to_onehot(dist, params=PARAMS):
        dstep = (params['DMAX'] - params['DMIN']) / params['DBINS']
        dbins = torch.linspace(params['DMIN']+dstep, params['DMAX'], params['DBINS'],dtype=dist.dtype,device=dist.device)
        db = torch.bucketize(dist.contiguous(),dbins).long()
    ...
    # # print(t0d.unsqueeze(2).unsqueeze(3).size())
    # t0d = t0d.unsqueeze(2).unsqueeze(3).expand(-1, -1, L, L, -1)
    idx = torch.arange(L, device=c6d.device).long().expand((B, L))
    sep = idx[:,None,:] - idx[:,:,None]
    sep = sep.abs() + torch.eye(L, device=c6d.device).unsqueeze(0)*999.9
    topk_matrix = torch.zeros((B, L, L), device=c6d.device)
    cond = torch.logical_or(topk_matrix > 0.0, sep < (L//4))
    b,i,j = torch.where(cond)
    topk_matrix[b,i,j]=1
    topk_matrix = topk_matrix.unsqueeze(1).unsqueeze(-1)
...
def c6d_to_t2d_dist_angle(c6d, params=PARAMS):
    def dist_to_onehot(dist, params=PARAMS):
        dstep = (params['DMAX'] - params['DMIN']) / params['DBINS']
        dbins = torch.linspace(params['DMIN']+dstep, params['DMAX'], params['DBINS'],dtype=dist.dtype,device=dist.device)
        db = torch.bucketize(dist.contiguous(),dbins).long()
    ...
    orien = torch.cat((torch.sin(c6d[...,1:-1]), torch.cos(c6d[...,1:-1])), dim=-1)*mask # (B, T, L, L, 6)
    # print(t0d.size())
